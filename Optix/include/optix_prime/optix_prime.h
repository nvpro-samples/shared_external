...
#define OPTIX_PRIME_VERSION 3063 /* 3.6.3 (major =  OPTIX_PRIME_VERSION/1000,     *
                                  *        minor = (OPTIX_PRIME_VERSION%1000)/10, *
                                  *        micro =  OPTIX_PRIME_VERSION%10        */

#ifndef RTPAPI
#  if defined( _WIN32 ) && !defined( RTP_STATIC )
...
extern "C" {
  ...
  */
  ...
  /**
   * @brief   Creates the acceleration structure over the triangles
   *
   * @ingroup Prime_Model
   *
   ...
   * may return before the update is finished. It is important that the vertex
   * and index buffers specified in @ref rtpModelSetTriangles not be modified
   ...
   *
   * The acceleration structure build performed by rtpModelFinish uses a fast,
   * high quality algorithm, but has the cost of requiring working memory of
   * roughly three times the size of the final acceleration structure. Also, the
   * final acceleration structure of each @ref RTPmodel must fit within two GB for
   * ray tracing on the GPU. The CPU has neither of these limits and upcoming
   * OptiX releases will lessen or remove these GPU limits. The application may catch
   * the out-of-memory exception and revert to a CPU-based @ref RTPmodel as a fallback.
   *
   * @param[in] model      Model
  ...
  */
   ...
   * @ingroup Prime_Model
   *
   * This function is intended to be used to copy a model from one OptiX Prime
   ...
   * @ingroup Prime_Model
   *
   * @ref rtpModelSetBuilderParameter sets builder parameters:
   *
   * If the value for @ref RTP_BUILDER_PARAM_USE_CALLER_TRIANGLES is
   * set to 0 (default), Prime computes additional data to improve
   * query performance and does not reference the user's vertex buffer
   * during a query. If set to 1, Prime uses the provided triangle
   * data as-is, resulting in slower query performance, but reduced
   * peak memory footprint.
   *
   * @ref RTP_BUILDER_PARAM_USE_CALLER_TRIANGLES requires an \a int type
   ...
   * If @ref RTP_BUILDER_PARAM_USE_CALLER_TRIANGLES is set to 0 peak
   * memory footprint may not actually be reduced when using smaller
   * chunks.
   *
   * @ref RTP_BUILDER_PARAM_CHUNK_SIZE requires an \a RTPsize type
   * pointer for the value.
   *
   * @param[in] model_api  Model
   * @param[in] param      Builder parameter to set
   * @param[in] size       Size of the parameter being set
   ...
   *
   */
  RTPresult RTPAPI rtpModelSetBuilderParameter( RTPmodel model_api, RTPbuilderparam param, RTPsize size, void* p );
